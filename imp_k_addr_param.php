...
class imp_k_addr_param extends \Area\Worker {
    public $workerID    = 521;
    ...
    public function run(){
        ...
        foreach ( GAR::Regions as $region ){
            ...
            for ( $i = 0 ; $i < $Z->numFiles ; $i++ ){
                if ( preg_match('#^'.$region.'/(?<name>AS_ADDR_OBJ_PARAMS_[\d]{8}_.*\.XML)$#i',$Z->getNameIndex($i),$M) ){
                    $foundIndex = $i;
                    break;
                }
            }
            if ( false === $foundIndex ){
                GARFn::lockWrite($this->workerID,__LINE__,'AS_ADDR_OBJ_PARAMS file not found for region '.$region);
                ...
                foreach ( $M as $data ) {
                    ...
                    switch ( intval(GARFn::rawXML_extract( $data[0] , 'TYPEID' )) ) {
                        ...
                        case 7: // ОКТМО
                            ...
                            $nUpdate++;
                            break;
                        case 11: // Код КЛАДР без признака актуальности
                            $this->db->mkUpdate($table_addr,[
                                'KLADR' => $VALUE
                            ],'OBJECTID',GARFn::rawXML_extract($data[0],'OBJECTID'));
                            $nUpdate++;
        ...
        // -> В таблице tmp**_addr заполнили OKATO, OKTMO на основе файлов AS_ADDR_OBJ_PARAMS_yyyymmdd_*.xml  //
        $this->setProcessedCount($this->proc)->setResult(0);
    }
}
new imp_k_addr_param();
