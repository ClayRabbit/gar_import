...
class imp_f_adm_hierarchy extends \Area\Worker {
    public $workerID    = 516;
    ...
    public function run(){
        ...
        foreach ( GAR::Regions as $region ){
            ...
            for ( $i = 0 ; $i < $Z->numFiles ; $i++ ){
                if ( preg_match('#^'.$region.'/(?<name>AS_ADM_HIERARCHY_[\d]{8}_.*\.XML)$#i',$Z->getNameIndex($i),$M) ){
                    $foundIndex = $i;
                    break;
                }
            }
            if ( false === $foundIndex ){
                GARFn::lockWrite($this->workerID,__LINE__,'AS_ADM_HIERARCHY file not found for region '.$region);
        ...
        foreach ( $list as list($region,$archive_filename,$filename) ){
            ...
            while(true){
                ...
                foreach ( $M as $data ) {
                    ...
                    // parent смотрим в таблице tmp**_addr, т.к. объекты в tmp**_house не могут быть владельцами объектов из addr или house
                    // NB!!! но могут быть владельцами, например, объектов apartments
                    if ( !$parent = $this->db->mkItem($table_addr,'id',['OBJECTID'=>$_PARENTOBJID]) )
                        continue;
                    $OBJECTID = GARFn::rawXML_extract($data[0],'OBJECTID');
                    if ( $obj = $this->db->mkItem($table_house,'id',['OBJECTID'=>$OBJECTID]) ){ // Сначала по house, их (совпадеинй) значительно больше
                        $this->db->mkUpdate($table_house,['owner_adm'=>$parent],'id',$obj);
                        $nChanges++;
                        continue;
                    }
                    if ( $obj = $this->db->mkItem($table_addr,'id',['OBJECTID'=>$OBJECTID]) ){
                        $this->db->mkUpdate($table_addr,['owner_adm'=>$parent],'id',$obj);
                        $nChanges++;
                        continue;
                    }
                }
        ...
        // -> В таблицах tmp**_addr , tmp**_house заполнен owner_adm на основе **/AS_ADM_HIERARCHY_yyyymmdd_*.xml  //
        $this->setProcessedCount($this->proc)->setResult(0);
    }
}
new imp_f_adm_hierarchy();
